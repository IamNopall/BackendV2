...
@section('content')
    <div class="max-w-3xl mx-auto mt-10 font-sans">
        ...
        <form action="{{ route('posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow-md border border-gray-200">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" value="{{ old('title', $post->title) }}" required class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>

            <!-- Content Field -->
            <div class="mb-4">
                <label class="block text-gray-700 font-semibold mb-2">Content</label>
                <textarea name="content" required class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500" rows="5">{{ old('content', $post->content) }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="tags" id="tags" value="{{ old('tags', implode(',', $post->tags->pluck('name')->toArray())) }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Enter tags, separated by commas" required>
            </div>
