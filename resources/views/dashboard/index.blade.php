...
@section('content')
    <div class="max-w-5xl mx-auto mt-10 font-sans">
        ...
        <!-- Menampilkan Tag Aktif jika ada -->
        @if (isset($tag))
            <div class="mb-6">
                <h2 class="text-lg font-medium text-gray-700">
                    Filtering by tag: <span class="text-blue-500">{{ $tag->name }}</span>
                </h2>
                <a href="{{ route('posts.index') }}" class="text-sm text-blue-500 hover:underline">Clear Filter</a>
            </div>
        @endif
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex flex-wrap items-center gap-2 mb-4">
                            @foreach ($post->tags as $tag)
                                <a href="{{ route('posts.filterByTag', $tag->id) }}" class="text-sm font-medium text-blue-600 bg-blue-100 px-3 py-1 rounded hover:bg-blue-200">
                                    {{ $tag->name }}
                                </a>
                            @endforeach
                        </div>
